...
namespace TShoes.UI.Modules
{
    ...
    {        
        ...
        {
            LoadSP(DBConTExt.Instance.GetAll_ChitietSP());
            DBConTExt.Instance.CTSP += LoadSP;
        ...
        private void btnTangSL1_Click(object sender, EventArgs e)
        {
            ...
            foreach (ChiTietSanPham item in DBConTExt.Instance.GetAll_ChitietSP())
            {
                if (item.ChitietSPID == billDetails.ChitietSP_ID && item.Soluong > 0)
                {
                    ...
                    item.Soluong -= 1;
                    DBConTExt.Instance.SaveChitietSP(item);
                    txtTienHang.Text = ListchitietTamThoi.Sum(x => x.Giaban * x.Soluong).ToString();
        ...
        private void btninbills_Click(object sender, EventArgs e)
        {
            ...
            if (print.ShowDialog() == DialogResult.Yes)
            {
                ...
                {
                    NhanVienID = DBConTExt.Instance.FindID_TenNV(txtTenNhanVien.Text),
                ...
                int billID = DBConTExt.Instance.SaveBills(bills);
                if (billID == 0)
                {
                ...
                foreach (BillDetails item in billDetailsBindingSource)
                {
                    ...
                    DBConTExt.Instance.SaveBillDetails(item);
                }
            ...
            foreach (ChiTietSanPham item in DBConTExt.Instance.GetAll_ChitietSP())
            {
                foreach (BillDetails billDetails in billDetailsBindingSource)
                {
                    if (item.ChitietSPID == billDetails.ChitietSP_ID)
                    {
                        item.Soluong += billDetails.Soluong;
                        DBConTExt.Instance.SaveChitietSP(item);
                        SanPham sanPham = DBConTExt.Instance.Tim_SP_TheoID(item.SanPham_ID.Value);
        ...
        {
            ChiTietSanPham sp = chiTietSanPhamBindingSource.Current as ChiTietSanPham;
            SanPham sanpham = DBConTExt.Instance.Tim_SP_TheoID(sp.SanPham_ID.Value);
            ...
            foreach (BillDetails chitiet in ListchitietTamThoi)
            {
                if (chitiet.ChitietSP_ID == sp.ChitietSPID)
                {
                    ...
                    DBConTExt.Instance.SaveChitietSP(sp);
        ...
        private void txtTimKiem_TextChanged(object sender, EventArgs e)
        {
            ...
            if (rbmasp.Checked)
            {
                sp = DBConTExt.Instance.GetAllCTSP_TheoMA(txtTimKiem.Text);
            }

            if (rbTenSP.Checked)
            {
                sp = DBConTExt.Instance.GetAllCTSP_TheoTen(txtTimKiem.Text);
            }
